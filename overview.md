...
> 📌 `v5.0.19`–`v5.0.27` 未逐版补记本文件，细节见 [`CHANGELOG.md`](CHANGELOG.md)（单一真相源）。

## v5.0.28（tag `v5.0.28` · seek 目标整秒契约守卫 —— 客户端侧**无功能变更**）

### 用户需求
- 「所有含拖拽的仓库，下发精度最小必须是 1 秒」——客户端要与 HA 卡片 / 网页 / 服务端同一条
  精度契约，并且**永久防回归**（不是改一次就完）。

### 背景（服务端事故的客户端侧意义）
- 服务端 sendspin 流式引擎按 **25ms 帧栅格**取帧（`lo = floor(pos/25) * 2400` 样点），而滑动窗口
  基准是「毫秒 → 样本」换算 —— **只有目标为 25ms 整数倍时两者严格相等**。HA 卡片 / 网页把当前位置
  原样下发（31.178s 这类毫秒精度）→ 取帧起点落在窗口基准之前 → 子进程进入**纯微任务自旋**
  （不 await I/O）→ 事件循环饿死 → 心跳 / poll RPC 全排不上队 → **65s 看门狗 SIGKILL**
  （现象：拖完进度条播放静默死掉、进度冻住）。
- **客户端一直是好的**：下发的是 `Duration.inSeconds`（截断整秒，1000 / 25 = 40 恒为帧栅格整数倍），
  所以从未触发。⇒ 客户端**不需要改实现**，需要的是把这条约定钉死。

### 实现
- **无 `lib/` 代码改动**（`cast_peer_provider.seek` 本就用 `position.inSeconds`；乐观值
  `smoothPositionSeconds` 与下发值同源，两半都是对的）。
- 新增守卫 `test/providers/cast_remote_progress_guard_test.dart` 的
  `group('seek 目标精度（最小粒度 1 秒）')` 3 例：
  ① 捕获**真正下发**的 `/seek` 请求体，断言 `seconds` 是 `int` 且 `Duration(milliseconds: 31178)` → 31
  （与生产同语义的截断）；② 多采样点（0 / 178 / 999 / 31178 / 62000 / 87033 / 108999 / 3599400 ms）
  满足 25ms 帧栅格不变式 + 与 `Duration.inSeconds` 同语义 + 代价 <1 秒；③ 乐观值与下发值同源。
- 已挂到**阻塞式** `playback-chain-guard.yml`（该文件本来就被它执行），即真门禁，无需新增 workflow。

### 验证
- `flutter test test/providers/cast_remote_progress_guard_test.dart` **11/11 通过**（含新增 3 例）。
- CI @`1149284`：**8/8 workflow 全 success**（含阻塞式 Playback Chain Guard 与最重的 Test Suite）。
- 本机 clone 曾落后 19 个提交（`131f1cf` → `d500ae7`），已 `bundle` 取齐后在新代码上重放，
  提交经 230 中转（本机无 GitHub 凭证），两端 blob sha 一致。

### 发版说明
- 本版**无功能变更**（与 v5.0.27 行为逐位一致），目的是让客户端也带上这条守卫、并产出可安装产物。
- 配套服务端 **v4.0.12**／HA 卡片 **v2.4.8**／HA 集成 **v2.0.5**（同一批次对齐）。
