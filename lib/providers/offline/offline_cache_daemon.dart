...
/// 一份待缓存的「播放就绪」任务快照。
class _PendingCacheJob {
  const _PendingCacheJob({
    required this.song,
    required this.queue,
    required this.index,
    this.upcomingSong,
...
class OfflineCacheDaemon {
  ...
  /// 串行执行期间收到的新任务只保留**最新**一份（单槽位），当前任务结束后立即续跑。
  /// 避免快速连切歌时 `if (_busy) return` 直接把最近播放的歌曲缓存丢弃。
  _PendingCacheJob? _pending;
  ...
  Future<void> onSongStartedOnline({
    ...
    if (_busy) {
      // 当前任务仍在整轨下载等：只记录最新请求，结束后优先补上，避免整首歌被丢。
      _pending = _PendingCacheJob(
        song: song,
        queue: queue,
        index: index,
  ...
  Future<void> _runJob(OfflineCacheManager cache, _PendingCacheJob job) async {
    try {
      // 封面小、速度快，先缓存当前曲封面，不被整轨下载阻塞。
      await _cacheSongCover(cache, job.song);
      await _cacheSongData(cache, job.song);
      // 预缓存实际「下一首将播放」的歌曲:随机模式由 player 侧 _resolveUpcomingSongForCache
      // 按随机语义取样给出,而非顺序队列的 index+1;未传时回退到顺序下一首。
      final next = job.upcomingSong ?? _nextFromQueue(job.queue, job.index);
