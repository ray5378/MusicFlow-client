...
播放目标分为两类：
- **后端投屏（链路 A，流转播放）**：DLNA / AirPlay / 群组 经主项目 `/rest/api/v1/peers*` 统一控制面，由后端推流，客户端只做控制面。
- **局域网 DLNA 直投（链路 B）**：客户端**自行 SSDP 发现 + SOAP 控制**局域网 DLNA 设备，——**A 档·直传**（把服务端直连流 URL 交给设备，设备作为 DMR 自拉流，客户端看门狗自动续播）。不再走本地中继/推流。

## 项目定位

- 客户端是 **MusicFlow 主项目后端的全量客户端**，接口参数、数据契约、行为表现一律以主项目为准（`docs/API.md` + `SPEC.md`）。
- 交互体验对标**网易云音乐 / QQ 音乐**：迷你播放条 → 全屏播放器（黑胶唱片）→ 队列面板 → 流转播放，操作手感、层级动效、卡片/列表呈现参考主流音乐 App。
...
- 后端投屏（链路 A，流转播放）：DLNA / 群组 / AirPlay 经主项目 `/rest/api/v1/peers*` 统一控制面，由后端推流
- **局域网 DLNA 直投（链路 B）**：
...
- **流转播放列表**（独立页）：把本机 + 所有在线播放端以圆形卡片铺成一页，圆内实时显示该端「此刻在播」的封面；**单击**圆切换遥控目标，**拖拽**圆把源队列流转给目标端（流转完成后遥控自动跟到目的地）
...
## 反向代理 / WAF 放行（流转播放与远程控制）

客户端会经反向代理对 Media 后端发起鉴权请求（OpenSubsonic 风格，带 `u/t/s` 签名），包括拉取对端播放状态、上报本地播放、拉流等。若反向代理开启了 **WAF 扫描（如 Coraza / OWASP CRS）**，部分请求可能被误拦，常见表现为：**流转播放页 / 弹窗看不到对端正在播放的歌名与封面，而本机自看正常**。

在 lucky 等反代入口添加「URI 前缀放行（豁免）」规则即可。注意：放行仅跳过 WAF 的规则扫描，**服务端 `u/t/s` 鉴权仍然生效**（未通过校验的请求仍会返回 401），因此不会绕过身份验证。

| Host 域名 | URI 前缀匹配 | URI 精确匹配 | 请求参数名 | 豁免规则ID |
| --- | --- | --- | --- | --- |
| your.domain.com | `/rest/` | （留空） | （留空） | （留空） |

若不想整段放行 `/rest/`，可改为分别放行以下更细的前缀：
...
以上覆盖「流转播放/远程控制」所需的全部请求：对端队列快照（`/peers/*/queue`）、本机状态上报（`local-status`、`queue/play`）、播放记录上报（`/rest/scrobble`）、音频拉流（`/rest/stream/*`）与封面（`/rest/getAlbumArt`）等。

> 说明：若仍偶发被拦，可在 WAF 拦截日志里找到带规则的拒绝（deny/403）记录，把对应规则 ID 填入「豁免规则ID」，即可只跳过那一条规则而不做整段放行。
